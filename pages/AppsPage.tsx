...
const pricingOptions = [{ value: 'All', label: 'All Pricing' },{ value: 'Free', label: 'Free' },{ value: 'FOSS', label: 'FOSS' },{ value: 'Paid', label: 'Paid' }];
const tagOptions = [
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex flex-col lg:flex-row gap-4 items-center">
                <div className="relative w-full flex-1">
                    ...
                    <input type="text" placeholder="Search apps by name or tag..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full h-10 pl-10 pr-10 bg-bg-secondary border border-border-secondary rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
                     {searchTerm && (<button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-0 flex items-center pr-3 text-text-tertiary hover:text-text-primary" aria-label="Clear search"><CloseIcon className="w-5 h-5" /></button>)}
                </div>
                <div className="w-full lg:w-auto flex items-center gap-4">
                    <button onClick={() => setSidebarOpen(true)} className="flex items-center gap-2 bg-bg-secondary border border-border-secondary rounded-full h-10 pl-3 pr-2 text-sm hover:bg-bg-hover">
                        <span className="text-text-tertiary">Category:</span>
                        <span className="text-text-primary font-medium truncate max-w-40">{currentCategoryLabel}</span>
                        <FilterListIcon className="w-6 h-6 p-1 bg-bg-active rounded-full" />
                    </button>
                    <div className="hidden md:block">
                        <Tabs options={pricingOptions} value={pricingFilter} onChange={setPricingFilter} className="h-10" />
                    </div>
                    <div className="hidden md:block">
                       <Tabs options={tagOptions} value={tagFilter} onChange={setTagFilter} className="h-10" />
                    </div>
                </div>
            </div>
