...
const RemoveWrapper: React.FC<RemoveWrapperProps> = ({ children, onRemove }) => (
	<div className="relative group">
		...
		<button
			...
			className="absolute -top-2 -right-2 z-10 p-1.5 rounded-full bg-bg-active text-text-secondary hover:bg-accent hover:text-bg-primary opacity-0 group-hover:opacity-100 shadow-sm"
			...
			<div className="px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="mb-6">
					<div className="flex items-center justify-between gap-3 mb-3 md:mb-0">
						<div className="flex items-center gap-3 min-w-0">
							<HeartIcon className="w-7 h-7 sm:w-8 sm:h-8 text-text-primary flex-shrink-0" />
							<h1 className="font-bold text-2xl sm:text-3xl text-text-primary truncate">Collections</h1>
						</div>

						{/* New Collection button - always visible on right */}
						<button
							onClick={() => setShowCreateModal(true)}
							className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-accent text-bg-primary rounded-md hover:opacity-90 flex-shrink-0"
						>
					...
					<div className="md:hidden">
						<DropdownMenu
							trigger={() => (
								<button className="w-full flex items-center justify-between gap-2 px-3 py-2 text-sm font-medium bg-bg-active text-text-primary rounded-md hover:bg-bg-active/80 border border-border-primary">
						...
						>
							{collectionNames.map(name => (
								<DropdownMenuItem
									...
									className={activeCollection === name ? 'bg-bg-active font-medium' : ''}
								>
									<span className="truncate">{name}</span>
									<span className="ml-auto text-xs text-text-secondary pl-2">
				...
				<div className="flex flex-col md:flex-row gap-6">
								...
								{collectionNames.map(name => {
									...
									return (
										<button
											key={name}
											onClick={() => setActiveCollection(name)}
											className={`w-full text-left group flex items-center justify-between gap-2 p-2 rounded-md text-sm ${activeCollection === name ? 'bg-bg-active' : 'hover:bg-bg-active/50'}`}
										>
											<span className={`flex-grow truncate ${activeCollection === name ? 'font-semibold text-text-primary' : 'text-text-secondary'}`}>
												{name}
											</span>
					...
					<main className="flex-grow min-w-0">
						<div className="pb-4 mb-4 border-b border-border-primary flex items-center justify-between gap-2">
							<h2 className="font-semibold text-xl sm:text-2xl text-text-primary truncate mr-2">{activeCollection}</h2>
							<div className="flex items-center gap-2">
								...
								{activeCollection !== 'Favorites' && (
									<div className="flex-shrink-0">
										<DropdownMenu
											trigger={() => (
												<button className="flex items-center justify-center h-9 w-9 text-text-secondary hover:bg-bg-active rounded-md border border-border-primary" aria-label="Options">
						...
						<div className="min-h-[50vh]">
							{activeItems.length > 0 ? (
								...
								<div className="flex items-center justify-center text-text-secondary py-16">
									<div className="text-center">
										<CollectionsIcon className="w-12 h-12 mx-auto text-text-secondary mb-4" />
										<p className="text-lg font-semibold text-text-primary">Collection is empty</p>
										<p className="text-sm mt-1 text-text-secondary">Right-click items to add them here</p>
									</div>
								</div>
							)}
						</div>
