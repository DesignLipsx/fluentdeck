		...
		<BaseModal isOpen={isOpen} onClose={onClose} ariaLabelledBy="confirm-delete-title">
			<>
				<div className="px-6 pt-6 pb-4 text-center">
					...
					<h2 id="confirm-delete-title" className="font-semibold text-lg text-text-primary">Delete Collection</h2>
					<p className="mt-2 text-sm text-text-secondary">
						Are you sure you want to delete the "{collectionName}" collection? This action cannot be undone.
					</p>
				</div>
				<div className="px-6 py-4 bg-bg-active border-t border-border-primary rounded-b-xl grid grid-cols-2 gap-3">
					<button onClick={onClose} className="px-4 py-2 text-sm font-medium rounded-md border border-border-primary hover:bg-border-primary text-text-secondary">
						Cancel
					</button>
					<button
