...
export const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, onClose, children }) => {
    ...
    return (
        <div
            ref={menuRef}
            className="fixed z-50 w-48 rounded-md shadow-lg bg-bg-secondary border border-border-primary focus:outline-none p-1 animate-scaleIn"
...
export const ContextMenuItem: React.FC<ContextMenuItemProps> = ({ children, onClick, className, disabled }) => (
    <button
        onClick={onClick}
        disabled={disabled}
        className={`w-full text-left text-text-primary group flex items-center px-3 py-2 text-sm rounded-md duration-150 ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-bg-active'} ${className}`}
        role="menuitem"
    >
        {children}
    </button>
...
export const ContextSubMenuTrigger: React.FC<ContextSubMenuTriggerProps> = ({ children, subMenu, className }) => {
    ...
    return (
        <div ref={itemRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="relative">
            <button
                className={`w-full text-left text-text-primary group flex items-center justify-between px-3 py-2 text-sm rounded-md hover:bg-bg-active duration-150 ${className}`}
                role="menuitem"
                aria-haspopup="true"
            ...
            {isOpen && (
                 <div
                     ...
                     className="fixed z-[51] w-56 max-h-60 overflow-y-auto rounded-md shadow-lg bg-bg-secondary border border-border-primary focus:outline-none p-1 animate-scaleIn"
                     style={{ top: position.top, left: position.left, opacity: position.opacity }}
                 >
                    <Suspense fallback={<div className="p-2 text-sm text-text-secondary">Loading...</div>}>
                        {subMenu}
                    </Suspense>
                </div>
            )}
