...
import { AddIcon, DownloadIcon, XIcon } from '@/components/Icons';
		...
		<>
			<div className="fixed z-50 bottom-4 inset-x-0 pointer-events-none animate-slide-up-fade">
				<div className="px-4 sm:px-6 lg:px-8 flex justify-center sm:justify-end">
					<div className="bg-bg-secondary border border-border-primary rounded-lg shadow-2xl flex items-center gap-2 p-2 pointer-events-auto w-full justify-center sm:w-auto sm:justify-start max-w-md sm:max-w-none">
						<button onClick={stopSelectionMode} className="p-1.5 rounded-full hover:bg-tab-active" aria-label="Cancel selection">
							<XIcon className="w-5 h-5" />
						</button>

						<span className="text-sm font-semibold pr-2 flex-shrink-0 text-text-primary">{selection.length}</span>
						...
						<DropdownMenu
							...
							menuClassName="w-56"
							trigger={() => (
								<button className="flex items-center justify-center gap-2 h-9 px-3 text-sm rounded-md bg-bg-active hover:bg-tab-active text-text-primary">
