...
function NetworkVisualization({ onNavigate }: NetworkVisualizationProps) {
  ...
  useEffect(() => {
    const updateSize = () => {
      ...
      setContainerSize({ width: maxWidth, height: maxWidth });
    };
  ...
  const allConnectionPoints: (number | 'center')[] = useMemo(() =>
    [...avatars.map(a => a.id), 'center'], [avatars]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const newConnection: Connection = {
        from, to, color: getRandomElement(['blue', 'green', 'purple', 'yellow', 'red', 'orange', 'pink', 'cyan', 'white'])
      };
  ...
  return (
    <div className="flex items-center justify-center p-2 sm:p-4 overflow-hidden font-sans w-full h-full min-h-[400px]">
      <div className="relative" style={{ width: `${dimensions.containerSize}px`, height: `${dimensions.containerSize}px` }}>
        <div className="absolute border-2 border-dashed border-purple-400/50 rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" style={{ width: `${dimensions.outerRadius * 2}px`, height: `${dimensions.outerRadius * 2}px` }}></div>
        <div className="absolute border-2 border-dashed border-purple-400/50 rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" style={{ width: `${dimensions.innerRadius * 2}px`, height: `${dimensions.innerRadius * 2}px` }}></div>
        
        <div className="absolute shadow-lg z-10 group cursor-pointer rounded-full bg-bg-secondary" style={{ left: `${dimensions.centerX}px`, top: `${dimensions.centerY}px`, transform: `translate(-50%, -50%)` }} onClick={onNavigate}>
            {topContributor ? (
                <img src={topContributor.avatar_url} alt={topContributor.login} className="rounded-full object-cover" style={{ width: `${dimensions.centerImageSize}px`, height: `${dimensions.centerImageSize}px` }} />
            ) : (
                <img src="https://i.postimg.cc/Vvs6wjNs/logo.png" alt="Fluent Deck Logo" className="rounded-full object-contain p-4" style={{ width: `${dimensions.centerImageSize}px`, height: `${dimensions.centerImageSize}px` }} />
            )}
            <div className={`absolute -top-10 left-1/2 transform -translate-x-1/2 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full font-medium text-gray-800 shadow-lg transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20 ${dimensions.tooltipTextSize} ${isCurrentlyConnected('center') ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
        ...
        <svg className="absolute inset-0 w-full h-full z-0">
          <defs>
            <filter id="lineGlow" x="-50%" y="-50%" width="200%" height="200%">
              ...
              <feComponentTransfer in="blur" result="glow"><feFuncA type="linear" slope="0.5" intercept="0" /></feComponentTransfer>
          ...
          {activeConnections.map((conn, i) => {
              ...
              return (
                <line key={i} x1={fromPos.cx} y1={fromPos.cy} x2={toPos.cx} y2={toPos.cy}
                  stroke={conn.color.includes('blue') ? '#3b82f6' : conn.color.includes('green') ? '#10b981' : conn.color.includes('purple') ? '#8b5cf6' : '#3b82f6'}
                  strokeWidth={dimensions.strokeWidth} strokeOpacity="0.8" fill="none"
                  style={{ strokeDasharray: lineLength, strokeDashoffset: lineLength, animation: 'draw-line 3s linear forwards' }}
                  filter="url(#lineGlow)"
                />
              );
          })}
        ...
        {[...outerRingAvatars, ...innerRingAvatars].map((avatar) => {
          ...
          return (
            <div key={avatar.id} className="absolute group" style={{ left: `${cx}px`, top: `${cy}px`, transform: `translate(-50%, -50%)` }} onClick={onNavigate}>
              <div className="bg-bg-secondary rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform duration-200 cursor-pointer relative" style={{ width: `${avatarSize}px`, height: `${avatarSize}px` }}>
                <div className="rounded-full relative overflow-hidden flex items-center justify-center" style={{ width: `${imageSize}px`, height: `${imageSize}px` }}>
                  <img src={avatar.image} alt={avatar.name} className="w-full h-full object-cover rounded-full" />
                </div>
                <div className={`absolute -top-10 left-1/2 transform -translate-x-1/2 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full font-medium text-gray-800 shadow-lg transition-opacity duration-200 pointer-events-none whitespace-nowrap z-20 ${dimensions.tooltipTextSize} ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                  {avatar.name}
                  <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-white/90"></div>
                </div>
              </div>
